...
from bot.helper.others.telegraph_helper import telegraph
from bot.helper.others.bot_utils import get_readable_file_size, setInterval
from bot.helper.others.fs_utils import get_mime_type, get_path_size
...
class GoogleDriveHelper:
    ...
    def __upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        file_metadata = {
            "name": file_name,
            "description": "Diunggah oleh ZBL Bot",
            "mimeType": mime_type,
        }
    ...
    def __copyFile(self, file_id, dest_id, new_name):
        body = {
            "parents": [dest_id],
            "name": new_name
        }
        ...
        try:
            ...
            if err.resp.get("content-type", "").startswith("application/json"):
                reason = (
                    jsnloads(err.content).get("error").get("errors")[0].get("reason")
                )
                if reason in ["userRateLimitExceeded", "dailyLimitExceeded"]:
                    if USE_SERVICE_ACCOUNTS:
                        if (
                            self.__sa_count == len(listdir("accounts"))
                            or self.__sa_count > 50
                        ):
    ...
    def clone(self, link, new_name):
        ...
        try:
            ...
            if "User rate limit exceeded" in str(err):
                ...
                if token_service is not None:
                    self.__service = token_service
                    return self.clone(link,new_name)
                msg = "File not found."
            else:
                msg = f"Error.\n{err}"
            return msg, ""
    ...
    def __cloneFolder(self, name, local_path, folder_id, parent_id):
        ...
        for file in files:
            if file.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                self.__copyFile(file.get("id"), parent_id, file.get("name"))
            if self.is_cancelled:
                break
    ...
    def drive_list(
        self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType=""
    ):
        msg = ""
        ...
        for index, parent_id in enumerate(DRIVES_IDS):
            ...
            response = self.__drive_query(
                parent_id, fileName, stopDup, isRecur, itemType
            )
        ...
        buttons = ButtonMaker()
        buttons.buildbutton("🔎 VIEW", f"https://{telegraph.api_url}/{path[0]}")
    ...
    def __download_folder(self, folder_id, path, folder_name):
        ...
        for item in result:
            ...
            elif not ospath.isfile(path + filename) and not filename.lower().endswith(
                tuple(EXTENTION_FILTER)
            ):
